...
### [JavaScript — references](references/javascript/README.md)

My authored explainers for JavaScript the language — the semantics shared by every runtime. The runtime (Node) is in ../node/ and the type layer in…

- [Destructuring, spread/rest, and optional syntax](references/javascript/destructuring-spread-and-optional-syntax.md) — Modern JavaScript has concise syntax that removes whole classes of boilerplate and bugs: destructuring pulls values out of objects/arrays (with defaults and…
- [Iterables, iterators, and generators](references/javascript/iterables-and-generators.md) — An iterable is any object with a [Symbol.iterator]() method (arrays, strings, Map, Set, arguments, NodeLists). That single protocol is what for...of, spread…
- [Promises and `async`/`await`](references/javascript/promises-and-async.md) — A Promise is a placeholder for a value that arrives later; it's pending, then settles fulfilled or rejected. async/await is syntax over promises: await pauses…
- [Prototypes and classes](references/javascript/prototypes-and-classes.md) — Every object has a hidden link to a prototype object; property lookups that miss walk up this prototype chain until they hit null. class is syntax sugar over…
- [Scope, closures, and hoisting](references/javascript/scope-closures-and-hoisting.md) — Use const by default and let when you must reassign; never var. Scope is lexical — an inner function can see the variables of the functions that enclose it,…
- [`this`, `call`/`apply`/`bind`](references/javascript/this-call-apply-bind.md) — In a regular function, this is decided by how the function is called, not where it's defined — there are four rules (new > explicit bind/call/apply >…
- [Types, values, and coercion](references/javascript/types-and-coercion.md) — JavaScript has 7 primitives (string, number, bigint, boolean, undefined, symbol, null) plus object (which includes arrays and functions). typeof reports the…
- [Value vs. reference, and copying](references/javascript/value-vs-reference-and-copying.md) — Primitives are copied by value; objects, arrays, and functions are handled by reference — the variable holds a pointer, so assigning or passing it shares the…
...
My authored explainers for Node.js — the runtime, its module systems, and package management. The TypeScript layer is in ../typescript/ and…
...
My authored explainers for TypeScript — the type system on top of JavaScript. The runtime (Node) is in ../node/; JavaScript-the-language is in ../javascript/.…
...
### [JavaScript — resources](resources/javascript/README.md)

Curated external material for JavaScript the language. My own explainers are in ../../references/javascript/; the runtime lives in the node domain and types in…
...
- [debounce.mjs](snippets/javascript/debounce.mjs) — debounce + throttle — the two rate-limiters you reach for constantly, with no
- [group-by.mjs](snippets/javascript/group-by.mjs) — Grouping a list by a key — the built-in Object.groupBy / Map.groupBy (Node 21+,
...
- [jsconfig.json](configs/javascript/jsconfig.json) — jsconfig.json — gives a PLAIN JavaScript project (no build step) editor-grade
...
- [Checklist: JavaScript correctness review](checklists/javascript/correctness-review.md) — Walk this over a JS diff (or before you commit) to catch the language's recurring footguns — coercion, this, async, and accidental mutation. Each item is…
...
- [Playbook: debug an async/promise bug](playbooks/javascript/debug-async-bug.md) — Trigger: code that "sometimes" fails, runs out of order, finishes before its data is ready, logs UnhandledPromiseRejection, or hangs forever. Est. time: 5–30…
